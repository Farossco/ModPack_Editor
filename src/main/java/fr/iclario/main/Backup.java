...
public class Backup
{
	...
	public static void manage ()
	{
		...
		switch (choice)
		{
			...
			case "5":
				...
				{
					if (thirdpartyIsPresent) return;

				}
				break;

			case "6":
				if (modpackIsPresent) if (thirdpartyIsPresent) create(Constants.thirdpartyFile);
				break;

			case "7":
				if (modpackIsPresent) if (thirdpartyIsPresent) return;
		}
		manage();
	}
	...
	public static void create (String file)
	{
		...
		try
		{
			if (new File(Constants.backupPath + backupFile).exists()) Files.delete(new File(Constants.backupPath + backupFile).toPath());
